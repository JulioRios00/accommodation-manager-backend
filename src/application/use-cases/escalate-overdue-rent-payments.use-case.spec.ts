...
function makeResident(): Resident {
  const r = new Resident();
  Object.assign(r, { id: 'resident-1', fullName: 'Ana Silva', email: 'ana@example.com', active: true });
  return r;
}
...
describe('EscalateOverdueRentPaymentsUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new EscalateOverdueRentPaymentsUseCase(paymentRepo, residentRepo, getTemplate, emailService);
  });

  it('day 0 (due today): sends nothing', async () => {
    store.set('payment-1', makePayment());
    const result = await useCase.execute(dateAt(DUE_DAY));
    expect(result).toEqual({ d1Sent: 0, d4Sent: 0, skippedNoEmail: 0 });
    expect(emailService.send).not.toHaveBeenCalled();
  });

  it('day 1: sends exactly one D+1 email and stamps d1ReminderSentAt', async () => {
    store.set('payment-1', makePayment());
    const result = await useCase.execute(dateAt(DUE_DAY + 1));
    expect(result).toEqual({ d1Sent: 1, d4Sent: 0, skippedNoEmail: 0 });
  ...
  it('day 4, still pending (D+1 already sent on day 1): sends exactly one D+4 email', async () => {
    ...
    expect(result).toEqual({ d1Sent: 0, d4Sent: 1, skippedNoEmail: 0 });
    expect(emailService.send).toHaveBeenCalledTimes(1);
  ...
  it('re-running the same day does not duplicate an already-sent D+1', async () => {
    ...
    expect(second).toEqual({ d1Sent: 0, d4Sent: 0, skippedNoEmail: 0 });
    expect(emailService.send).not.toHaveBeenCalled();
  });
  ...
  it('skips (and counts) a payment whose resident has no email, without crashing the run', async () => {
    residentRepo.findById.mockResolvedValue(Object.assign(new Resident(), { id: 'resident-1', fullName: 'No Email', email: null }));
    store.set('payment-1', makePayment());
    const result = await useCase.execute(dateAt(DUE_DAY + 1));
    expect(result).toEqual({ d1Sent: 0, d4Sent: 0, skippedNoEmail: 1 });
    expect(emailService.send).not.toHaveBeenCalled();
  });
});
